...
# Parameters
#------------------------------------------------------------------------------
INPUT_FILE = os.path.join('../data/int_output', 'dewk_wl_counts_raw.txt')
WL_INPUT_FILE = os.path.join('../data/int_output', 'de_wishlist_no_freq.txt')
OUTPUT_FILE = os.path.join('../data/output', 'dewk_wishlist_freq.txt')
NROWS = None
...
wl_df = pd.read_csv(WL_INPUT_FILE, sep='\t', na_filter=False,
                    quoting=csv.QUOTE_NONE, nrows=NROWS)
wl_df = wl_df.merge(df.drop(['section'], axis=1), on='headword',
